                    ...
                    <?php foreach ($user as $u) : ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="" class="badge badge-success" data-toggle="modal" data-target="#modaleditmenu<?= $u['id']; ?>">Reset</a>
                            </td>
                        </tr>
                        <?php $i++; ?>
                    <?php endforeach; ?>
...
<?php foreach ($user as $u) : ?>
    <div class="modal fade" id="modaleditmenu<?= $u['id']; ?>" tabindex="-1" aria-labelledby="modaleditmenulabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="modaleditmenu">Reset PW</h1>
                    <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="<?= base_url('admin/resetpw'); ?>" method="post">
                    <div class="modal-body">
                        <div class="form-group">
                            <input type="hidden" name="id" id="id" value="<?= $u['id']; ?>">
                            <input type="hidden" class="form-control" id="password" name="password" value="<?= $u['username']; ?>">
                        </div>
                    </div>
